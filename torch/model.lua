...
function create_network()
  ...
  for i = 1,params.conv_layers do
    ...
    local b = nn.ReLU()(a)
    --local c = nn.SpatialConvolutionMM(nfeats[i], nfeats[i], filtsize, filtsize)(b)
    --local d = nn.ReLU()(c)
    x_conv[i] = nn.SpatialMaxPooling(poolsize,poolsize,poolsize,poolsize)(b)
  end

  --local out_size = nfeats[2]*10*10
  ...
  local g = nn.ReLU()(nn.Linear(params.g_size,params.g_size)(nn.CMulTable()({g_x, g_l})))

  --local h  = {[0] = nn.ReLU()(nn.Linear(params.g_size,params.g_size)(g))}
  local h  = {[0] = g}
