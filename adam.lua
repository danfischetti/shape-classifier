...
function Adam:step()
	...
	self.m:mul(b1_t)
	self.m:add(self.gparams,1-b1_t)
	self.v:mul(self.beta2)
	self.v:addcmul(1-self.beta2,self.gparams,self.gparams)
	local m_bias = torch.mul(self.m,1-b1_t)
	local v_bias = torch.mul(self.v,1-self.beta2)
	self.params:addcdiv(-self.alpha,m_bias,torch.sqrt(v_bias):add(self.epsilon))
end
