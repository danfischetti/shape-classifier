...
local filepath = '/Volumes/RAM Disk/renders/'
...
function get_view(l,n)
  json.rpc.call('http://localhost:9090','get_view',{l[1],l[2],l[3],l[4],n-1})
  return image.load(filepath .. 'm' .. n-1 .. '.png')
end
...
local function fp(y)
  ...
  for i = 1, params.n_glimpses do
    for j = 1,params.batch_size do
      x[j]:copy(get_view(model.l[i-1][j],j))
    end
...
local function bp(y)
  ...
  for i = params.n_glimpses-1, 1, -1 do
    ...
    s_err = model.networks[params.n_glimpses]:backward({x,l,s},{a_err,l_err,model.ds,b_err})[3]
    g_replace_table(model.ds, s_err)
    -- cutorch.synchronize()
  end
  ...
  for i = 1,10 do
    for j = 1,params.n_batches do
      ...
      for k = 1,params.batch_size do
        y[k]=model.classes[j+((k-1)%14)*100+math.floor((k-1)/14)]
      end
      local err=fp(y)
      print('Test err ' .. err)
      bp(y)
